...
async function passwordHash(password: string) { const salt = randomBytes(16).toString('hex'); return `${salt}:${(await scrypt(password, salt, 64)).toString('hex')}`; }
async function matches(password: string, saved: string) { const [salt, expected] = saved.split(':'); const actual = (await scrypt(password, salt, 64)).toString('hex'); return timingSafeEqual(Buffer.from(actual, 'hex'), Buffer.from(expected, 'hex')); }
