...
export const payments = pgTable('payments', {
  ...
  check('payment_method_valid', sql`${t.paymentMethod} IN ('COD', 'BANK_TRANSFER')`),
  check('payment_status_valid', sql`${t.status} IN ('PENDING', 'PAY_ON_DELIVERY', 'CHARGED', 'REFUNDED', 'FAILED')`),
  check('payment_provider_required', sql`${t.status} NOT IN ('CHARGED', 'REFUNDED', 'PAY_ON_DELIVERY') OR ${t.providerTransactionId} IS NOT NULL`),
