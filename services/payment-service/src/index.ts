...
const server = serve({ fetch: app.fetch, port }, (info) => {
  console.log(`payment-service listening on port ${info.port} (local simulated provider: ${mode})`);
});
const messaging = new RabbitWorker(pool, 'payment', participantHandler(pool, 'payment',
  ...
  onError: () => console.error('payment messaging temporarily unavailable or message rejected; pending work retained'),
});
messaging.start();
let stopping = false;
...
server.on('error', (error: NodeJS.ErrnoException) => {
  console.error(`payment-service could not start: ${error.code ?? 'server error'}`);
