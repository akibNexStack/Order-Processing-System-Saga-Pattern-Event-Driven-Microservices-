...
import {
  CreateShipmentCommandSchema,
  CancelShipmentCommandSchema,
  ResultSchema,
  commandFingerprint,
  commandKey,
...
// The ShippingService class manages the execution of shipping commands, including creating and canceling shipments. It interacts with a ShippingProvider to perform the actual operations and uses a PostgreSQL database to handle idempotency, order state management, and command receipts. The service ensures that commands are processed in a consistent manner, handling potential conflicts and providing appropriate results based on the outcomes of the operations.
export class ShippingService {
  ...
  async execute(input: ShippingCommand, commit?: ResultCommit): Promise<Result> {
    const parsed =
      input.operation === 'CREATE_SHIPMENT'
        ? CreateShipmentCommandSchema.parse(input)
        : CancelShipmentCommandSchema.parse(input);
    const command: ShippingCommand = {
      ...parsed,
      orderId: parsed.orderId.toLowerCase(),
      sagaId: parsed.sagaId.toLowerCase(),
      ...(parsed.operation === 'CREATE_SHIPMENT'
        ? {
            payload: {
              ...parsed.payload,
    ...
    try {
      ...
      locked = (
        await client.query('select pg_try_advisory_lock(hashtextextended($1,0)) as locked', [lock])
      ).rows[0].locked;
      if (!locked)
        return unknown(command, 'Another operation is processing this order; retry the same key');
      ...
      const receipt = await db.transaction(async (tx) => {
        ...
        const [row] = await tx
          ...
          .where(eq(commandReceipts.idempotencyKey, command.idempotencyKey));
        return row;
      });
      if (receipt.fingerprint !== fingerprint)
        return failure(
          ...
          'This key was used with a different command',
        );
      ...
      const [shipment] = await db
        .select()
        .from(shipments)
        .where(eq(shipments.orderId, command.orderId));
      const [cancellation] = await db
        .select()
        .from(shipmentCancellations)
        .where(eq(shipmentCancellations.orderId, command.orderId));
      let result: Result;
      if (
        (shipment && shipment.sagaId !== command.sagaId) ||
        (cancellation && cancellation.sagaId !== command.sagaId)
      ) {
        ...
        if (
          ...
          const existing =
            ...
            (
              await db
                .insert(shipments)
                .values({ orderId: command.orderId, sagaId: command.sagaId, ...command.payload })
                .returning()
            )[0];
        ...
        await db
          .insert(shipmentCancellations)
          .values({ orderId: command.orderId, sagaId: command.sagaId, shipmentId: shipment?.id })
          .onConflictDoNothing();
        if (cancellation?.status === 'CANCELLED' || cancellation?.status === 'NOOP') {
          ...
          if (createResult?.outcome === 'UNKNOWN') {
            result = unknown(
              command,
              'Original shipment creation is unresolved; retry cancellation with the same key',
            );
          } else if (
            !shipment ||
            shipment.status === 'FAILED' ||
            createResult?.outcome === 'FAILED'
          ) {
            ...
            result = resultFor(
              ...
              await this.callProvider({
                ...command,
                idempotencyKey: commandKey(command.sagaId, command.operation),
              }),
            );
          }
        }
      ...
      await db.transaction(async (tx) => {
        if (
          command.operation === 'CANCEL_SHIPMENT' &&
          result.outcome === 'SUCCEEDED' &&
          result.operation === 'CANCEL_SHIPMENT'
        ) {
          await tx
            .update(shipmentCancellations)
  ...
  private async resolveCreate(db: NodePgDatabase, shipment: Shipment): Promise<Result> {
    ...
    const command = CreateShipmentCommandSchema.parse({
      version: 1,
      orderId: shipment.orderId,
      sagaId: shipment.sagaId,
      idempotencyKey: commandKey(shipment.sagaId, 'CREATE_SHIPMENT'),
    ...
    if (result.outcome !== 'UNKNOWN') {
      await db
        .update(shipments)
        .set({
          status: result.outcome === 'SUCCEEDED' ? 'CREATED' : 'FAILED',
          providerShipmentId:
            result.outcome === 'SUCCEEDED' && result.operation === 'CREATE_SHIPMENT'
              ? result.data.providerShipmentId
              : null,
          createResult: result,
          updatedAt: new Date(),
        })
    ...
    try {
      const invocation =
        command.operation === 'CREATE_SHIPMENT'
          ? this.provider.create(command)
          : this.provider.cancel(command);
      const raw = await Promise.race([
        invocation,
        new Promise<Result>((resolve) => {
          timer = setTimeout(
            () => resolve(unknown(command, 'Provider deadline exceeded; retry the same key')),
  ...
  async status(orderId: string) {
    ...
    const [row] = await db
      .select({ shipment: shipments, cancellation: shipmentCancellations })
      .from(shipments)
