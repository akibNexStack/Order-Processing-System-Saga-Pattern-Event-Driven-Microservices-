...
export type LogEvent = { event: string; orderId?: string; sagaId?: string; messageId?: string; operation?: string;
  outcome?: string; reason?: string; leaseToken?: string; route?: string; requestId?: string; durationMs?: number; userId?: string };
...
export function structuredLogger(service: string, write: (line: string) => void = console.log): LogSink {
  return entry => {
    ...
    const { event, orderId, sagaId, messageId, operation, outcome, reason, leaseToken, route, requestId, durationMs, userId } = entry;
    write(JSON.stringify({ timestamp: new Date().toISOString(), service, event, orderId, sagaId, messageId, operation, outcome, reason, leaseToken, route, requestId, durationMs, userId }));
  };
}
